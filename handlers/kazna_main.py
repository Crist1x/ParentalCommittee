...
async def my_tasks(message: Message):
    if kazna_chack(message.from_user.username):
        ...
        kazna_data = cursor.execute(f"SELECT school, class, letter FROM admin WHERE username = '{message.from_user.username}'").fetchone()
...
Обязательность: {my_tasks[0][4]}""", reply_markup=my_tasks_ikb)
            ...
            await message.answer("Список целей пуст. Создайте новую в главном меню", reply_markup=keyboards.reply.greeting_admin)
...
async def delete_task(message: Message):
    if kazna_chack(message.from_user.username):
        ...
        cursor = connection.cursor()
        kd = cursor.execute(
            f"SELECT school, class, letter FROM admin WHERE username = '{message.from_user.username}'").fetchone()
        ...
        connection.commit()
        cursor.close()
        await message.answer("Цель была успешно удалена", reply_markup=keyboards.reply.greeting_admin)
